...
public class getRemoteImage : IHttpHandler
{
    ...
    {
        string savePath = context.Server.MapPath("~/upload/images/" + DateTime.Now.Year + "/" + DateTime.Now.Month + "/" + DateTime.Now.Day + "/");       //保存文件地址
        ...
        try
        {
            for (int i = 0, len = imgUrls.Length; i < len; i++)
            {
                ...
                tmpNames.Add("../../../upload/images/" + DateTime.Now.Year + "/" + DateTime.Now.Month + "/" + DateTime.Now.Day + "/" + tmpName);
            }
        }
        catch (Exception)
        {
